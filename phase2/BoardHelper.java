...
public class BoardHelper {
    ...
    public static ArrayList<Point> getReversePoints(int[][] board,int player,int i,int j){
        ...
        if(mi<=7 && board[mi][mj] == player && points.size()>0){
            allReversePoints.addAll(points);
        }
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if(board[i][j]==oplayer){
                    ...
                    ArrayList<Point> possibility = new ArrayList<>();

                    //up
                    if(i>0 && board[i-1][j]==0) possibility.add(new Point(i-1,j));
                    //down
                    if(i<7 && board[i+1][j]==0) possibility.add(new Point(i+1,j));
                    //right
                    if(j<7 && board[i][j+1]==0) possibility.add(new Point(i,j+1));
                    //left
                    if(j>0 && board[i][j-1]==0) possibility.add(new Point(i,j-1));
                    //up-left
                    if(i>0 && j>0 && board[i-1][j-1]==0) possibility.add(new Point(i-1,j-1));
                    //up-right
                    if(i>0 && j<7 && board[i-1][j+1]==0) possibility.add(new Point(i-1,j+1));
                    //down-left
                    if(i<7 && j>0 && board[i+1][j-1]==0) possibility.add(new Point(i+1,j-1));
                    //down-right
                    if(i<7 && j<7 && board[i+1][j+1]==0) possibility.add(new Point(i+1,j+1));

                    //remove duplicates
                    for(Point pf : possibility){
                        boolean redundant = false;
